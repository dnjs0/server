...
public class Ex01 {
    public static void main(String[] args) {
        /*
          ...
          - 영속성 계층(Persisitence Layer) > 역할
         - 중간 계층 > 위치
         - Java 라는 언어를 사용해서 관계형 데이터를 접근 + 조작해주는 계층
         - 각종 API > 클래스들을 제공
         ...
         JDBC Driver 설치
         - 관련 클래스 가져오기 > *.jar 참조
         - 어떤 DB 사용? > 오라클 사이트 접속 > JDBC 구현해놓은 jar제공 > 다운로드 + 사용
         - Oracle 설치 > Driver 같이 배포
         - 빌드 도구를 통해서 참조 > Maven, Gradle 등...
         
         
         jar 참조하기 : 플젝 우클릭 > bulid path > configure > library > classpath > addjars > ojdbc11.jar 참조
     ...
     */
        ...
        try {
            ...
            //JDBC 드라이버 로딩
            //무조건 해야하는 전처리 과정(이해할필요 굳이X)
            Class.forName("oracle.jdbc.driver.OracleDriver");
            
            //DB연결
            //- DriverManager가 DB연결을 하고 > 연결정보를 가지고 있는 Connection 객체를 반환한다.
